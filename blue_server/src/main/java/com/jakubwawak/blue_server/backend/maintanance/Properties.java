...
public class Properties {
    ...
    public void createPropertiesFile(){
        try{
            ...
            writer.write("#blue_server properties file\n");
            writer.write("$fileVersion=1\n");
            writer.write("#by Jakub Wawak / all rights reserved / kubawawak@gmail.com\n");
            writer.write("$serverName=myBlueBitch\n");
            writer.write("$databaseString=mongodb://localhost:27017\n");
            writer.write("$newUserPost=true\n");
            writer.write("$publicNotes=true\n");
            writer.write("$accountCreation=true\n");
            writer.write("#secret for signing JWT token sessions - SHOULD BE 256 BIT\n");
            writer.write("$blueSecret=\n");
            writer.write("#time for token expiration in milliseconds\n");
            writer.write("$tokenExpiration=3600000\n");
            writer.close();
        }catch(Exception ex){
            error = true;
